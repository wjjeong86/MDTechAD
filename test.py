...
for _ in range(10):
    ...
    D = np.clip(np.abs((I-P)*1+0.0),0,1)
    D2 = np.clip(np.abs((S-P)*1+0.0),0,1)
    sss = np.hstack( (I,P,S,D,D2) )
#     imshow(cv2.resize(sss,(256*5,256))*255)
#     print( cword[0])
    
    P3 = cv2.ximgproc.guidedFilter(guide=(S+P)/2,src=P,radius=7,eps=0.001**2)
    P3 = np.expand_dims(P3,-1)
    D3 = np.clip(np.abs((S-P3)*1+0.0),0,1)
